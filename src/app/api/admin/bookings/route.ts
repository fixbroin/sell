...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const rawBookings = await getDocsInternal(pool, 'bookings', [{ type: 'orderBy', field: 'createdAt', direction: 'desc' }]);
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const pool = await getPool();
    await updateDocInternal(pool, 'bookings', id, data);
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const pool = await getPool();
    await deleteDocInternal(pool, 'bookings', id);
